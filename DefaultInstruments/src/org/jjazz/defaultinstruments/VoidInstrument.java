...
/**
 * A special "void" instrument: no bank change or program change is associated to this instrument. When used the system should not
 * send any Midi bank select or program change messages for this instument.
 */
@NbBundle.Messages(
        {
...
public class VoidInstrument extends Instrument implements Serializable
{
    ...
    static protected VoidInstrument getInstance()
    {
        synchronized (VoidInstrument.class)
        {
            if (INSTANCE == null)
            {
                INSTANCE = new VoidInstrument(0, CTL_VoidInstrument());
            }
        }
        return INSTANCE;
    }
