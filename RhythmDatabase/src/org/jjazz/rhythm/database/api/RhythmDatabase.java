...
/**
 * The RhythmDatabase is the central place to get information about installed
 * rhythms and get Rhythm instances.
 * <p>
 * Implementation should populate the database upon creation.
 * <p>
 * RhythmInfo instances are used to describe the available rhythms. They can be
 * serialized by a Rhythmdatabase implementation in a cache file to avoid
 * requesting all the Rhythm instances upon each startup -this is very time
 * consuming when hundreds of rhythm files are used. Use
 * getRhythmInstance(RhythmInfo) to get the Rhythm instance.
 */
public interface RhythmDatabase
{
     ...
     * <p>
     * If rhythmId contains 2 instances of the
     * AdaptedRhythm.RHYTHM_ID_DELIMITER, then this id represents an
     * AdaptedRhythm which is created on demand, see
     * AdaptedRhythm.getUniqueId(). The rhythm provider, the original rhythm and
     * the time signature are obtained from rhythmId, and the returned rhythm
     * instance is obtained by calling
     * RhythmProvider.getAdaptedRhythmInstance(Rhythm, TimeSignature). Rhythm
     ...
     * <p>
     * This might result in a lengthy operation (e.g. because of file reading).
    ...
    /**
     * Try to provide the rhythm instance which is an adapted version of r for a
     * different time signature.
     * <p>
     * If the adapted rhythm could be obtained it is added in the database.
     * AdaptedRhythm instances are cached.
     *
     * @param r
     * @param ts
     * @return Can be null if no adapted rhythm is available.
     * @throws IllegalArgumentException If ts is the time signature of r, or if
     * r is not a rhythm of this database.
     */
     ...
     * @param rp
     * @return
     * @exception IllegalArgumentException If rp is not a RhythmProvider
     * available.
     */
    List<RhythmInfo> getRhythms(RhythmProvider rp);

    /**
     * Try to find a rhythm in the database which is "similar" to the specified
     * rhythm info.
     * <p>
     * "Similar" means at least share the same time signature. The
     * implementation could for example use RhythmFeatures.getMatchingScore() to
     * help identify the most similar rhythm.
     *
     * @param ri
     * @return A "similar" rhythm which at least share the same timesignature.
     * Null if nothing relevant found.
     */
    ...
    /**
     * @return The list of TimeSignature for which we have at least 1 rhythm in
     * the database
     */
     ...
     *
     * @param ts TimeSignature
     * @return Can not be null, but there is no guarantee that
     * getRhythmInstance() on the returned value will work (e.g. if this
     ...
     * @param ts TimeSignature
     * @param ri
     * @exception IllegalArgumentException If rhythm is not part of this
     * database or if ri is an AdaptedRhythm
     */
    ...
    /**
     * Force a rescan of all the RhythmProviders available in the lookup to add
     * rhythms in the database.
     * <p>
     * Rescan is programmed to be performed at next application startup. If
     * might be done immediatly if the immediate parameter is true and if the
     * implementation supports it.
     * <p>
     * Note: once added in the database, a RhythmProvider and its Rhythms can't
     * be removed (until program restarts).<br>
     * Fire a change event if database has changed after the
     * forceRescanUponStartup.
     *
     * @param immediate If true try to rescan immediatly (without waiting for a
     * restart).
     */
     ...
     * <p>
     * Add new rhythms to a populated database. Fire a change event after
