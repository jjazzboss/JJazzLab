...
public class CopyBuffer
{
    ...
    public List<SongPart> get(SongStructure targetSgs, int targetStartBarIndex)
    {
        ...
        {
            // Normal case
            // Retrieve only a spt if we can find the same parentSection or an equivalent with same name/timesignature.
            for (SongPart spt : sptBuffer)
            {
                CLI_Section parentSection = spt.getParentSection();
                ...
                {
                    int nbBars = targetCls.getSectionSize(newParentSection);
