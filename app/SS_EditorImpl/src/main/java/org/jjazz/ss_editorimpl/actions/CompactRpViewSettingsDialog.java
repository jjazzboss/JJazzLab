...
public class CompactRpViewSettingsDialog extends javax.swing.JDialog
{
     ...
     */
    public Map<Rhythm, List<RhythmParameter<?>>> getResult()
    {
        Map<Rhythm, List<RhythmParameter<?>>> res = new HashMap<>();
        if (tblModel != null)
        {
            for (int i = 0; i < tblModel.rhythms.size(); i++)
            {
                ...
                res.put(r, rps);
                for (int j = 0; j < tblModel.uniqueRpIds.size(); j++)
                {
                    var rpId = tblModel.uniqueRpIds.get(j);
                    if (tblModel.data[i][j])
                    {
                        rps.add(getRpFromId(r.getRhythmParameters(), rpId));
                    }
                }
            }
        }
    ...
    private RhythmParameter<?> getRpFromId(List<RhythmParameter<?>> rps, String id)
    {
        return rps.stream().filter(rp -> rp.getId().equals(id)).findAny().orElse(null);
    }
    ...
    /**
     * This method is called from within the constructor to setModel the form. WARNING: Do NOT modify this code. The content of this method is always
    ...
    public class MyModel extends AbstractTableModel
    {
        ...
        List<Rhythm> rhythms;
        boolean[][] data;
        List<String> uniqueRpIds = new ArrayList<>();
        int colCount;
        int rowCount;
        ...
        public MyModel(List<Rhythm> rhythms, SS_Editor editor)
        {
            ...
            for (var r : rhythms)
            {
                ...
                {
                    if (!uniqueRpIds.contains(rp.getId()))
                    {
                        uniqueRpIds.add(rp.getId());
                    }
                }
            }
            rowCount = uniqueRpIds.size();


            // Update data
            data = new boolean[rhythms.size()][uniqueRpIds.size()];
            for (int i = 0; i < rhythms.size(); i++)
            {
                ...
                for (int j = 0; j < uniqueRpIds.size(); j++)
                {
                    data[i][j] = getRpFromId(compactViewRps, uniqueRpIds.get(j)) != null;
                }
            }
        }
        ...
        public Object getValueAt(int row, int col)
        {
            if (col == COL_RP)
            {
                return getRhythmParameter(row, col).getDisplayName();
            } else
            {
                return data[col - COL_FIRST_RHYTHM][row];
            }
        }
        ...
        {
            return col >= COL_FIRST_RHYTHM && getRhythmParameter(row, col) != null;
        }
         ...
         * @param row
         * @param col
         * @return Can be null if col >= COL_FIRST_RHYTHM
         */
        private RhythmParameter<?> getRhythmParameter(int row, int col)
        {
            String rpId = uniqueRpIds.get(row);
            if (col == COL_RP)
            {
                // Return first rhythm parameter found
                for (var r : rhythms)
                {
                    var res = getRpFromId(r.getRhythmParameters(), rpId);
                    if (res != null)
                    {
                        return res;
                    }
                }
                throw new IllegalStateException("rpId=" + rpId + " rhythms=" + rhythms);
            } else
            {
                return getRhythm(col).getRhythmParameters().stream()
        ...
        private Rhythm getRhythm(int col)
        {
            return rhythms.get(col - COL_FIRST_RHYTHM);
        }
