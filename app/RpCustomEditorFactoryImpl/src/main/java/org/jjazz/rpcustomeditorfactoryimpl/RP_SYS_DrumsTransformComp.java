...
public class RP_SYS_DrumsTransformComp extends RealTimeRpEditorComponent<RP_SYS_DrumsTransformValue> implements ListDataListener, PropertyChangeListener, PlaybackListener
{
    ...
    public void preset(RP_SYS_DrumsTransformValue rpValue, SongPartContext sptContext)
    {
        ...
        Runnable task = () -> 
        {
            ...
            BaseSongSession tmpSession = new BaseSongSession(workContext, false, false, false, 0, null, false);
            try
            {
    ...
    /**
     * We don't directly use the PhraseTransform as data, because there might be several identical PhraseTransforms in a chain, so using
     * index and list provides a unique instance to be transferred.
     */
