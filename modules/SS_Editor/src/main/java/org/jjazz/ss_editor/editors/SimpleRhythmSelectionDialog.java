 ...
 *  Contributor(s):  
 */
package org.jjazz.ss_editor.editors;
...
import static org.jjazz.ss_editor.editors.Bundle.*;
...
@NbBundle.Messages(
        {
            ...
            "CTL_PreferredTempo=Tempo",
            "CTL_File=File",
            "CTL_Version=Version",
            "CTL_Vendor=Vendor",
            "CTL_Feel=Feel",
            "CTL_Filename=File name"
        })
public class SimpleRhythmSelectionDialog extends RhythmSelectionDialog implements ListSelectionListener
{
    ...
    private void updateRhythmInfo(RhythmInfo ri)
    {
        ...
        tm.setValueAt(CTL_PreferredTempo(), 2, 0);
        tm.setValueAt(ri.getPreferredTempo(), 2, 1);

        tm.setValueAt(CTL_PreferredTempo(), 3, 0);
        tm.setValueAt(tr.getName() + " [" + tr.getMin() + "~" + tr.getMax() + "]", 3, 1);

        tm.setValueAt(CTL_File(), 4, 0);
...
//
//        tm.setValueAt(CTL_Parameters(), 5, 0);
...
//        tm.setValueAt(sb.toString(), 5, 1);
        ...
        tm.setValueAt(CTL_Version(), 6, 0);
        tm.setValueAt(ri.getVersion(), 6, 1);

        tm.setValueAt(CTL_Vendor(), 7, 0);
