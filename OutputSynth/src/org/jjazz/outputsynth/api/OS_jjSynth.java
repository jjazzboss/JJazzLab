...
/**
 * The builtin OutputSynth for jjSynth (based on OS_JJazzLabSoundFont_XG since FluidSynth can handle XG bank select).
 */
public class OS_jjSynth extends OutputSynth
{


    private static OS_jjSynth INSTANCE;
    private final MidiSynth midiSynth;
    private static final Logger LOGGER = Logger.getLogger(OS_jjSynth.class.getSimpleName());

    public static OS_jjSynth getInstance()
    {
        synchronized (OS_jjSynth.class)
        {
            if (INSTANCE == null)
            {
                INSTANCE = new OS_jjSynth();
            }
        }
        return INSTANCE;
    }
