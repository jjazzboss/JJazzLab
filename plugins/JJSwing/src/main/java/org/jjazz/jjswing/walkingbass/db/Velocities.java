...
public class Velocities
{
     ...
     */
    public final static IntRange VELOCITY_STATS_MIN_MAX = new IntRange(35, 92);
    /**
     * Statistics performed on WbpSources notes (with length >0.6f). June 2025
     */
    public final static int VELOCITY_STATS_MEDIAN = 62;
    /**
     * Statistics performed on WbpSources notes (with length >0.6f). June 2025
     */
    public final static float VELOCITY_STATS_STDDEV = 7.63f;

    /**
     * The main normal velocity range for bass notes.
     */
    public final static IntRange VELOCITY_MAIN_RANGE = new IntRange(55, 70);
    public final static int VELOCITY_EXTENDED_OFFSET = 6;
    /**
     * The extended velocity range for bass notes.
     *
     * @see #VELOCITY_MAIN_RANGE
     * @see #VELOCITY_EXTENDED_OFFSET
     */
    public final static IntRange VELOCITY_EXTENDED_RANGE = VELOCITY_MAIN_RANGE.getTransformed(-VELOCITY_EXTENDED_OFFSET, VELOCITY_EXTENDED_OFFSET);
     ...
     */
    static public boolean normalizeVelocities(Phrase p, float durMin)
    {
        boolean b = false;
        ...
        if (!VELOCITY_MAIN_RANGE.contains(velRange))
        {
            int offset = (int) Math.round((stats.getAverage() - VELOCITY_STATS_MEDIAN) / 2);
            b = offset != 0;
            p.processNotes(ne -> ne.getDurationInBeats() >= durMin, ne -> ne.setVelocity(VELOCITY_EXTENDED_RANGE.clamp(ne.getVelocity() - offset)));
