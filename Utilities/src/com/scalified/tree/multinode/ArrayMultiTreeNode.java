...
/**
 * Implementation of the K-ary (multi node) tree data structure, based on the resizable array representation
 *
 * @author shell
 * @version 1.0.0
 * @since 1.0.0
...
public class ArrayMultiTreeNode<T> extends MultiTreeNode<T>
{
    ...
    /**
     * Default initial branching factor, that is the number of subtrees this node can have before getting resized
     */
    private static final int DEFAULT_BRANCHING_FACTOR = 10;

    /**
     * The maximum size of array to allocate. Some VMs reserve some header words in an array. Attempts to allocate larger arrays
     * may mResult in OutOfMemoryError: Requested array size exceeds VM limit
     */
     ...
     *
     * @param data data to store in the current tree node
     * @param branchingFactor initial branching factor, that is the number of subtrees the current tree node can have before
     * getting resized
     */
    ...
    /**
     * Increases the capacity of the subtrees array, if necessary, to ensure that it can hold at least the number of subtrees
     * specified by the minimum subtrees capacity argument
     *
    ...
    /**
     * Increases the subtrees array capacity to ensure that it can hold at least the number of elements specified by the minimum
     * subtrees capacity argument
     *
     * @param minSubtreesCapacity the desired minimum subtrees capacity
     */
    ...
    /**
     * Returns the index of the first occurrence of the specified subtree within subtrees array; {@code -1} if the subtrees array
     * does not contain such subtree
     *
     * @param subtree subtree to find the index of
     * @return index of the first occurrence of the specified subtree within subtrees array; {@code -1} if the subtrees array does
     * not contain such subtree
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Adds the collection of the subtrees with all of theirs descendants to the current tree node
     * <p>
     * Checks whether this tree node was changed as a result of the call
     *
     * @param subtrees collection of the subtrees with all of their descendants
     * @return {@code true} if this tree node was changed as a result of the call; {@code false} otherwise
     */
    @Override
    ...
    /**
     * Returns the collection of nodes, which have the same parent as the current node; {@link Collections#emptyList()} if the
     * current tree node is root or if the current tree node has no subtrees
     * <p>
     * Overridden to have a faster array implementation
     *
     * @return collection of nodes, which have the same parent as the current node; {@link Collections#emptyList()} if the current
