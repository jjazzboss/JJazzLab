 ...
 * <h1>Definition</h1>
 * A tree data structure can be defined recursively (locally) as a collection of nodes (starting at a root node), where each node
 * is a data structure consisting of a value, together with a list of references to nodes (the children), with the constraints
 * that no reference is duplicated, and none points to the root
 * <p>
 * A tree is a (possibly non-linear) data structure made up of nodes or vertices and edges without having any cycle. The tree with
 * no nodes is called the <b>null</b> or
 * <b>empty</b> tree. A tree that is not empty consists of a root node and potentially many levels of additional nodes that form a
 * hierarchy
 * <h1>Terminology</h1>
 * <ul>
 ...
 * <li><b>Parent</b> - a node, other than the root, which is connected to other successor nodes</li>
 ...
 * <li><b>Node Height</b> - the number of edges on the longest downward path between that node and a leaf</li>
 * <li><b>Tree Height</b> - the number of edges on the longest downward path between the root and a leaf (root height)</li>
 ...
 * <li><b>Labeled Tree</b> - tree in which a label or value is associated with each node of the tree</li>
 * <li><b>Expression Tree</b> - tree which specifies the association of an expression�s operands and its operators in a uniform
 * way, regardless of whether the association is required by the placement of parentheses in the expression or by the precedence
 * and associativity rules for the operators involved</li>
 * <li><b>Branching Factor</b> - maximum number of children a node can have</li>
 * <li><b>Pre order</b> - a form of tree traversal, where the action is called firstly on the current node, and then the pre order
 * function is called again recursively on each of the subtree from left to right</li>
 * <li><b>Post order</b> - a form of tree traversal, where the post order function is called recursively on each subtree from left
 * to right and then the action is called</li>
 * </ul>
 *
 * @author shell
 * @version 1.0.0
...
public abstract class TreeNode<T> implements Iterable<TreeNode<T>>, Serializable, Cloneable
{
    ...
    /**
     * Returns the collection of the child nodes of the current node with all of its proper descendants, if any
     * <p>
     * Returns {@link Collections#emptySet()} if the current node is leaf
     *
     * @return collection of the child nodes of the current node with all of its proper descendants, if any;
     ...
     * @return {@code true} if this tree node was changed as a result of the call; {@code false} otherwise
     */
     ...
     * @return {@code true} if the current tree node was changed as a result of the call; {@code false} otherwise
     */
    public abstract boolean dropSubtree(TreeNode<T> subtree);

    /**
     * Removes all the subtrees with all of its descendants from the current tree node
     */
     ...
     *
     * @return {@code true} if the current tree node is root of the tree; {@code false} otherwise
     */
     ...
     *
     * @return root node of the current node; itself, if the current node is root
     */
     ...
     *
     * @return parent node of the current node; {@code null} if the current node is root
     */
    ...
    /**
     * Checks whether the current tree node is a leaf, e.g. does not have any subtrees
     *
     * @return {@code true} if the current tree node is a leaf, e.g. does not have any subtrees; {@code false} otherwise
     */
    ...
    /**
     * Searches the tree node within the tree, which has the specified data, starting from the current tree node and returns the
    ...
    /**
     * Searches the tree nodes within the tree, which have the specified data, starting from the current tree node and returns the
     * collection of the found tree nodes
     *
    ...
    /**
     * Checks whether among the current tree node subtrees there is a specified subtree
     *
     * @param subtree subtree whose presence within the current tree node children is to be checked
     * @return {@code true} if among the current tree node subtrees there is a specified subtree; {@code false} otherwise
     */
    ...
    /**
     * Get the first direct child subtree whose data equals the specified parameter.
     *
     * @param data
    ...
    {
        for (var mSubtree : subtrees())
        {
            if (Objects.equals(mSubtree.data(), data))
            {
                return mSubtree;
            }
        }
    ...
    /**
     * Checks whether the current tree node with all of its descendants (entire tree) contains the specified node
     *
     * @param node node whose presence within the current tree node with all of its descendants (entire tree) is to be checked
     * @return {@code true} if the current node with all of its descendants (entire tree) contains the specified node;
    ...
    /**
     * Checks whether the current tree node with all of its descendants (entire tree) contains all of the nodes from the specified
     * collection (the place of nodes within a tree is not important)
     *
     * @param nodes collection of nodes to be checked for containment within the current tree node with all of its descendants
     * (entire tree)
     * @return {@code true} if the current tree node with all of its descendants (entire tree) contains all of the nodes from the
    ...
    /**
     * Removes the first occurrence of the specified node from the entire tree, starting from the current tree node and traversing
    ...
    /**
     * Removes all of the collection's nodes from the entire tree, starting from the current tree node and traversing in a pre
     * order manner
     * <p>
     * Checks whether the current tree node was changed as a result of the call
     *
    ...
    /**
     * Traverses the tree in a pre ordered manner starting from the current tree node and performs the traversal action on each
     * traversed tree node
     *
    ...
    /**
     * Traverses the tree in a post ordered manner starting from the current tree node and performs the traversal action on each
     * traversed tree node
     *
     * @param action action, which is to be performed on each tree node, while traversing the tree
     */
    ...
    /**
     * Returns the pre ordered collection of nodes of the current tree starting from the current tree node
     *
     * @return pre ordered collection of nodes of the current tree starting from the current tree node
     */
    ...
    /**
     * Returns the post ordered collection of nodes of the current tree starting from the current tree node
     *
     * @return post ordered collection of nodes of the current tree starting from the current tree node
     */
     ...
     * @throws TreeNodeException exception that may be thrown in case if the current node does not have such descendant or if the
     ...
     *
     * @param node node, which the common ancestor is determined for, along with the current node
     * @return common ancestor of the current node and the node specified
     * @throws TreeNodeException exception that may be thrown in case if the specified tree node is null or the specified tree
     * node does not belong to the current tree or if any of the tree nodes either the current one or the specified one is root
     */
    ...
    /**
     * Checks whether the current tree node is a sibling of the specified node, e.g. whether the current tree node and the
     * specified one both have the same parent
     *
     * @param node node, which sibling with the current tree node is to be checked
     * @return {@code true} if the current tree node is a sibling of the specified node, e.g. whether the current tree node and
     * the specified one both have the same parent; {@code false} otherwise
     */
     ...
     *
     * @param node node, which is checked to be the descendant of the current tree node
     * @return {@code true} if the current tree node is the ancestor of the node specified; {@code false} otherwise
     */
     ...
     *
     * @param node node, which is checked to be the ancestor of the current tree node
     * @return {@code true} if the current tree node is the ancestor of the node specified; {@code false} otherwise
     */
    ...
    /**
     * Returns the height of the current tree node, e.g. the number of edges on the longest downward path between that node and a
     * leaf
     *
     * @return height of the current tree node, e.g. the number of edges on the longest downward path between that node and a leaf
     */
    public int height()
    {
        if (isLeaf())
    ...
    /**
     * Returns the depth (level) of the current tree node within the entire tree, e.g. the number of edges between the root tree
     * node and the current one
     *
     * @return depth (level) of the current tree node within the entire tree, e.g. the number of edges between the root tree node
     * and the current one
     */
    public int level()
    {
    ...
    /**
     * Links the specified parent tree node reference as the parent to the specified tree node
     *
    ...
    /**
     * Checks whether there is at least one not {@code null} element within the input collection
     * <pre>
     ...
     * @return {@code true} if there is at least one not {@code null} element within the input collection; {@code false} otherwise
     */
    ...
    /**
     * Checks whether the specified collection is @{code null}, empty or if all of its elements are {@code null}
     * <pre>
     ...
     * @param collection input collection to check
     * @param <T> type of the data, which parametrises collection
     * @return {@code true} if the specified collection is {@code null}, empty or if all of its elements are {@code null};
     * {@code false} otherwise
     */
    ...
    /**
     * Base tree node iterator, which is expected to be extended by {@link TreeNode} subclasses in order to perform custom
     * implementation and return it in {@link #iterator()}
     */
    protected abstract class TreeNodeIterator implements Iterator<TreeNode<T>>
    {

        /**
         * An expected size of the tree node required to check whether the tree node was changed during <b>foreach</b>
         * iteration
         */
        ...
        /**
         * Indicates whether there is a next tree node available within iteration
         */
        private boolean nextNodeAvailable = true;

        /**
         * Returns the leftmost node of the current tree node if the current tree node is not a leaf
         *
         * @return leftmost node of the current tree node if the current tree node is not a leaf
         * @throws TreeNodeException an exception that is thrown in case if the current tree node is a leaf
         */
        protected abstract TreeNode<T> leftMostNode();

        /**
         * Returns the right sibling node of the current tree node if the current tree node is not root
         *
         * @return right sibling node of the current tree node if the current tree node is not root
         * @throws TreeNodeException an exception that may be thrown in case if the current tree node is root
         */
        protected abstract TreeNode<T> rightSiblingNode();

        /**
         * Checks whether the current tree node is not a leaf and returns the leftmost node from {@link #leftMostNode()}
         *
         * @return leftmost node of the current tree node if the current tree node is not a leaf
         * @throws TreeNodeException an exception that is thrown in case if the current tree node is a leaf
         */
        ...
        /**
         * Checks whether the current tree node is not root and returns the right sibling node from {@link #rightSiblingNode()}
         *
         * @return right sibling node of the current tree node if the current tree node is not root
         * @throws TreeNodeException an exception that may be thrown in case if the current tree node is root
         */
        ...
        /**
         * Returns {@code true} if the iteration has more elements; otherwise returns {@code false}
         *
         ...
         * iteration and throws {@link ConcurrentModificationException} exception if so
         */
        ...
        /**
         * Removes from the underlying tree the last element returned by this iterator (optional operation)
         * <p>
         * This method can be called only once per call to {@link #next}. The behavior of an iterator is unspecified if the
         * underlying tree is modified while the iteration is in progress in any way other than by calling this method
         *
         * @throws IllegalStateException an exception that may be thrown in case if remove was performed without any iteration
         * @throws TreeNodeException an exception that may be thrown in case if remove was performed on a root node
         */
        @Override
        public void remove()
        {
