 ...
 * <p>
 * a) an entire component<br>
 * b) a region of the component<br>
 * c) the entire desktop<br>
 * d) a region of the desktop<br>
 * <p>
 * The first two use the Swing paint() method to draw the component image to the BufferedImage. The latter two use the AWT Robot to create
 * the BufferedImage.
 * <p>
 * The created image can then be saved to a file by usig the writeImage(...) method. The type of file must be supported by the ImageIO write
 * method.
 * <p>
 * Although this class was originally designed to create an image of a component on the screen it can be used to create an image of
 * components not displayed on a GUI. Behind the scenes the component will be given a size and the component will be layed out. The default
 * size will be the preferred size of the component although you can invoke the setSize() method on the component before invoking a
 * createImage(...) method. The default functionality should work in most cases. However the only foolproof way to get a image to is make
 * sure the component has been added to a realized window with code something like the following:
 * <p>
 * JFrame frame = new JFrame();<br>
 * frame.setContentPane( someComponent );<br>
 * frame.pack();<br>
 * ScreenImage.createImage( someComponent );<br>
 * <p>
