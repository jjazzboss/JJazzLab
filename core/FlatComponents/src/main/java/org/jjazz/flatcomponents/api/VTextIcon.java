...
public class VTextIcon implements Icon, PropertyChangeListener
{
     ...
     * <p>
     * from http://www.unicode.org/unicode/reports/tr9/tr9-3.html When setting text using the Arabic script in vertical lines, it
     * is more common to employ a horizontal baseline that is rotated by 90� counterclockwise so that the characters are ordered
     * from top to bottom. Latin text and numbers may be rotated 90� clockwise so that the characters are also ordered from top to
     * bottom.
     * <p>
     * Rotation rules - Roman can rotate left, right, or none - default right (counterclockwise) - CJK can't rotate - Arabic must
     * rotate - default left (clockwise)
     * <p>
     * from the online edition of _The Unicode Standard, Version 3.0_, file ch10.pdf page 4 Ideographs are found in three sections
     * of the Unicode Standard... U+4E00-U+9FFF, U+3400-U+4DFF, U+F900-U+FAFF
     * <p>
     * Hiragana is U+3040-U+309F, katakana is U+30A0-U+30FF
     * <p>
     * from http://www.unicode.org/unicode/faq/writingdirections.html East Asian scripts are frequently written in vertical lines
     * which run from top-to-bottom and are arrange columns either from left-to-right (Mongolian) or right-to-left (other
     * scripts). Most characters use the same shape and orientation when displayed horizontally or vertically, but many
     * punctuation characters will change their shape when displayed vertically.
     * <p>
     * Letters and words from other scripts are generally rotated through ninety degree angles so that they, too, will read from
     * top to bottom. That is, letters from left-to-right scripts will be rotated clockwise and letters from right-to-left scripts
     * counterclockwise, both through ninety degree angles.
     * <p>
     * Unlike the bidirectional case, the choice of vertical layout is usually treated as a formatting style; therefore, the
     * Unicode Standard does not define default rendering behavior for vertical text nor provide directionality controls designed
     * to override such behavior
     * <p>
     */
