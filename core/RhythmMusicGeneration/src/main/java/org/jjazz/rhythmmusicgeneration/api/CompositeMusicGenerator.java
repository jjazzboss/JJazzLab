...
public class CompositeMusicGenerator implements MusicGenerator
{
     ...
     * @param rpVariationValue An optional destination rhythm's variation to be used by mg. If null, reuse the source variation.
     ...
     *
     * @param mg
     * @param mapBaseRvMgDelegate Associates a MgDelegate for some base RhythmVoices (possibly all). All MgDelegates must use mg.
     * @param subContext
    ...
    private Map<RhythmVoice, Phrase> callGenerator(MusicGenerator mg, Map<RhythmVoice, MgDelegate> mapBaseRvMgDelegate, SongContext subContext) throws MusicGenerationException
    {
        ...
        {
            var uniqueDelegateRvs = mapBaseRvMgDelegate.values().stream()
                    .peek(mgt -> 
                    {
                        if (mgt.mg != mg)
                        {
                            throw new IllegalArgumentException("mgt=" + mgt);
                        }
                    }
                    )
                    .map(mgt -> mgt.rv)
            ...
            Rhythm delegateRhythm = (Rhythm) uniqueDelegateRvs.iterator().next().getContainer();
            SongContext delegateContext = subContext;      // by default     
            if (delegateRhythm != baseRhythm)
            {
                // We need a new delegateContext with a song using delegateRhythm instead of baseRhythm
                delegateContext = createDelegateContext(subContext, delegateRhythm, mapBaseRvMgDelegate);
            }
     ...
     * @param mapBaseRvMgDelegate The MgDelegates must have the same rpVariation value
     * @return
     * @throws org.jjazz.rhythm.api.MusicGenerationException
     */
    private SongContext createDelegateContext(SongContext context, Rhythm delegateRhythm, Map<RhythmVoice, MgDelegate> mapBaseRvMgDelegate) throws MusicGenerationException
    {
        Preconditions.checkArgument(!mapBaseRvMgDelegate.isEmpty());
        var mgDelegates = mapBaseRvMgDelegate.values();
        var rpVariationValue = mgDelegates.iterator().next().rpVariationValue;
        if (!mapBaseRvMgDelegate.values().stream()
                .allMatch(mgd -> Objects.equals(mgd.rpVariationValue, rpVariationValue)))
        {
            throw new IllegalArgumentException("mapBaseRvMgDelegate=" + mapBaseRvMgDelegate);
        }
