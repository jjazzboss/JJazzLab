...
public class CompositeMusicGenerator implements MusicGenerator
{
     ...
     * @param rpVariationValue The delegate rhythm's variation to be used by mg. Will be ignored if delegate rhythm does not use
     *                         the RP_SYS_Variation parameter.
     * @param postProcessor    An optional post-processor of the phrase generated by mg. If null no postprocessing is done. For
     *                         example this can be used to harmonize notes velocities.
     */
...
//        }
    ...
    /**
     * Process multi-variation DelegateUnits from a single MgConfig in order to find new MgConfigs with consistent groups of
     * mono-variation DelegateUnits.
     * <p>
     * A "consistent group" of DelegateUnits means that they all use the same variation value for the same RhythmVoices in 1 or
     * more SongParts. Smallest group is 1 DelegateUnit.
     *
    ...
    /**
     * Get all the MgConfigs with their associated RvDelegateUnits -possibly with multiple delegate variations for a given
     * SongPart.
     *
     * @param baseRhythmSpts
     * @param rhythmVoices
    ...
    private Map<RhythmVoice, Phrase> callDelegateGenerator(SongContext songContext, MgConfig mgConfig, Map<SongPart, String> mapSptRpVariationValue) throws MusicGenerationException
    {
        ...
        // Map each Phrase back to its source RhythmVoice
        for (var baseRv : mgConfig.mapRvBaseDest.keySet())
        {
            var targetRv = mgConfig.mapRvBaseDest.get(baseRv);
            if (baseRv != targetRv)
            {
                Phrase pSrc = mapRvPhrases.get(targetRv);
                if (pSrc == null)
                {
                    LOGGER.log(Level.SEVERE, "callDelegateGenerator() Unexpected pSrc=null for baseRv={0} targetRv={1}. Using empty phrase instead.", new Object[]
                    {
                        baseRv, targetRv
                    });
                    LOGGER.log(Level.SEVERE, "   mgConfig.mg={0}", mgConfig.mg().getClass().getSimpleName());
                    LOGGER.log(Level.SEVERE, "   mgConfig.mapRvBaseDest:");
                    LOGGER.log(Level.SEVERE, Utilities.toMultilineString(mgConfig.mapRvBaseDest(), "   "));
                    for (var rv : mapRvPhrases.keySet())
                    {
                        var p = mapRvPhrases.get(rv);
                        var pStr = p == null ? "null" : p.size() + " notes";
                        LOGGER.log(Level.SEVERE, "   mapRvPhrases'{'{0}'}'={1}", new Object[]
                        {
                            rv, pStr
                        });
                    }
        ...
        // Remove all non-baseRhythm RhythmVoices
        mapRvPhrases.keySet().removeIf(rv -> rv.getContainer() != baseRhythm);
     ...
     * <p>
     * Changes: <br>
     * - bar range is limited to the bar range of the impacted SongParts (note that bar range might include non processed
     * SongPart(s), they will be silenced later).<br>
